...
for (f in 1:length(theDatalist)){
  ...
  sinu_dist_map = read.csv(sprintf('%s/%s.csv',data_path,strsplit(theDatalist[f],".csv")[[1]]))
  covariate_im_sinu <- as.im(sinu_dist_map, c(as.numeric(max(sinu_dist_map$x)),as.numeric(max(sinu_dist_map$y)),1,1))
