...
for s in range(len(strct_grp)):
    if (strct_grp[s] == 'bone') | (strct_grp[s] == 'fat'):
        continue
    strct_name = strct_grp[s]
    sub_xlpath = '{}/{}'.format(xl_path,strct_name)
    ...
    for i in range(len(MPN_subs)):
        subsavepath = '{}/{}/{}'.format(savepath,strct_name,MPN_subs[i])
        ...
        for ii in range(len(mpn_idx)):
            ...
            x_data = pd.read_excel('{}/{}/{}.xlsx'.format(xl_path,strct_name,ID_list[mpn_idx[ii]]))
            ...
            for iii in range(len(cluster_type_list)):
                ...
                dist_grp = np.array(x_data['Distance to {}'.format(strct_name.capitalize())])[type_idx]
                # pdb.set_trace()
            ...
            print('{} - {}: {} out of {} permutation has been done'.format(strct_name,MPN_subs[i], ii+1,len(mpn_idx)))
        ...
        # combined_p_value, combined_z_score = sstouffers_method(bone_pvalue[0])
